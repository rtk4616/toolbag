...
" Default configuration functions {{{
	...
	function! EasyMotion#InitMappings(motions) "{{{
		for motion in keys(a:motions)
            " ======================================
            " MIKE WILKERSON UPDATE IN THIS FOR-LOOP
            " ======================================
            "
			call EasyMotion#InitOptions({ 'mapping_' . motion : '' })
			call EasyMotion#InitOptions({ 'mapping_' . 'f' : g:EasyMotion_leader_key })
		endfor
		...
		try
			...
			" Shade inactive source {{{
				if g:EasyMotion_do_shade
					let shade_hl_pos = '\%' . line('w0') . 'l\%'. 1 .'c'
